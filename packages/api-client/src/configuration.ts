...
export async function loginBilreg(
  ...
  if (!response.ok || !envelope.success || envelope.data.status !== 'success') {
    throw new ApiClientError(
      (envelope.success && envelope.data.message) || `Login failed with status ${response.status}`,
      response.status,
  ...
  if (!parsed.success) {
    throw new ApiClientError(`Login response validation failed: ${parsed.error.message}`, response.status)
  }
  return parsed.data
}
...
export function createConfigurationApi(client: AdmissionQueueClient) {
  ...
  return {
    ...
    getWorkstation(workstationKey: string) {
      return client.getJson(`${base}/workstations/${encodeURIComponent(workstationKey)}`, workstationSchema)
    },
    ...
    getDisplay(displayId: string) {
      return client.getJson(`${base}/displays/${encodeURIComponent(displayId)}`, queueDisplaySchema)
    },
    ...
    listAudit(page = 1, pageSize = 50) {
      return client.getJson(`${base}/audit`, configurationAuditPageSchema, { page, pageSize })
    },
  }
}
